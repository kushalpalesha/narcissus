...
ExecutionContext.prototype = {
    ...
    putValue: function (v, w, vn) {
      if (v instanceof Reference) {
        if (typeof v.base === "function" && v.propertyName === "prototype") {
          var fint = functionInternals.get(v.base);
          definitions.defineProperty(fint, "prototype", w, false);
        }
    ...
    evalIfBlock: function (cond, thenPart, elsePart) {
        var x = ExecutionContext.current;
        if (cond) {
          execute(thenPart, x);
        } else if (elsePart) {
    ...
    evalDot: function(t, r, c, u, n) {
      return new Reference(toObject(t, r, c[0]), u, n);
    },
    evalFunctionCall: function(f, n, r, c, a) {
      var x = ExecutionContext.current;
      x.staticEnv = n.staticEnv;
      if (typeof f !== "function") {
        throw new TypeError(r + " is not callable", c[0].filename, c[0].lineno);
      }
      var t = (r instanceof Reference) ? r.base : null;
      if (t instanceof Activation)
        t = null;
      return call(f, t, a, x);
    },
    runWhileLoop: function(whileCond, n) {
      var x = ExecutionContext.current;
      while (whileCond) {
        try {
          execute(n.body, x);
        ...
        whileCond = !n.condition || getValue(execute(n.condition, x));
      }
    }
};
...
function execute(n, x) {
    ...
    switch (n.type) {
      ...
      case IF:
        var cond = x.getValue(execute(n.condition, x));
        x.evalIfBlock(cond, n.thenPart, n.elsePart);
        break;
      ...
      case FOR:
        ...
        // FALL THROUGH
      case WHILE:
        var whileCond = !n.condition || x.getValue(execute(n.condition, x));
        x.runWhileLoop(whileCond, n);
        break;
      ...
      case MOD:
        c = n.children;
        var v1 = x.getValue(execute(c[0], x));
        var v2 = x.getValue(execute(c[1], x));
    ...
    case INSTANCEOF:
      ...
      case DECREMENT:
        ...
        if (n.postfix)
            v = u;
        var newVal = (n.type === INCREMENT) ? x.evalBinOp(u, 1, ops[PLUS]) : x.evalBinOp(u, 1, ops[MINUS]);
        x.putValue(t, newVal, n.children[0]);
        if (!n.postfix)
            v = newVal;
        break;
      ...
      case CALL:
        ...
        a = execute(c[1], x);
        f = x.getValue(r);
        v = x.evalFunctionCall(f, n, r, c, a);
        break;
...
function newFunction(n, x) {
    ...
    var proto = {};
    definitions.defineProperty(p, "prototype", proto, false);
