...
FacetExecContext.prototype.runWhileLoop = function (whileCond, n) {
  var x = FacetExecContext.current;
  evaluateEach(whileCond, function(whileCond,x) {
    while (whileCond) {
      try {
        interpreter.execute(n.body, x);
      } catch (e if e === BREAK_SIGNAL && x.target === n) {
        break;
      } catch (e if e === CONTINUE_SIGNAL && x.target === n) {
        // Must run the update expression.
      }
      n.update && x.getValue(interpreter.execute(n.update, x));
      whileCond = !n.condition || x.getValue(interpreter.execute(n.condition, x), getPC());
    }
  }, x);
};

FacetExecContext.prototype.evalFunctionCall = function(f, n, r, c, a) {
  //FIXME: function call with faceted arguments not supported.
  var x = FacetExecContext.current;
  return evaluateEachPair(f, r, function(f, r, x) {
    x.staticEnv = n.staticEnv;
    if (typeof f !== "function") {
      throw new TypeError(r + " is not callable", c[0].filename, c[0].lineno);
    }
    var t = (r instanceof interpreter.Reference) ? r.base : null;
    if (t instanceof interpreter.Activation)
      t = null;
    return interpreter.call(f, t, a, x);
  }, x);
};

FacetExecContext.prototype.evalDot = function(t, r, c, u, n) {
    var execContext = FacetExecContext.current;
    return evaluateEach(t, function (t, x) {
      return new interpreter.Reference(interpreter.toObject(t, r, c[0]), u, n);
    }, execContext);
};
...
/**
 * Applies the function "f" to both facets of the value v.
 *
 * @param  {*} v [description]
...
/**
 * PolicyEnvironment: Policy Agnostic Programming environment
 * policyMap stores all policies associated with labels
 */
function PolicyEnvironment() {
  this.policyMap = {};
}

PolicyEnvironment.prototype = {
  /**
   * Create label and associate a default "true" policy to it.
   * @param labelName
   * @returns {Label}
   */
  mkLabel: function (labelName) {
    var label = new Label(labelName, false);
    if (!(label in this.policyMap)) {
      this.policyMap[label] = function() { return false; }
    }
    return label;
  },
  /**
   * Add the `label`:`policy` association to the policyMap.
   * FIXME: does not completely conform to the evaluation rules
   * @param label
   * @param policy
   */
  ...
  /**
   * Create a sensitive value in the form of a faceted value.
   * @param label
   * @param privateValue
   * @param publicValue
   * @returns {*}
   */
  mkSensitive: function (label, privateValue, publicValue) {
    if (!label)
      throw new Error('Must specify a principal.');
    if (!(label instanceof Label)) {
      label = this.mkLabel(label);
    }
    return cloak(label, privateValue, publicValue);
  },
  /**
   * This will resolve policies for all principals in the facetedValue based on the given context object.
   * @param context
   * @param {FacetedValue} val
   * @returns {*}
   */
  ...
  /**
   * This method will try to resolve the policy only for the label at the head of the pc of the facetedValue based on the
...
function isFaceted(val) {
  return val instanceof FacetedValue;
}

// Functions that can be used by programs to incorporate faceted Behaviour.
definitions.defineProperty(facetedGlobalBase, "PolicyEnvironment", PolicyEnvironment, true, true);
definitions.defineProperty(facetedGlobalBase, "isFaceted", isFaceted, true, true);
