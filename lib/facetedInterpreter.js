...
PolicyLibrary.prototype = {
  ...
  restrict: function(label, policy) {
    if (typeof policy === "function" && typeof policy() === "boolean") {
      ...
      // Exit if policy is not of type boolean
      print("Policy is not a funciton or return type of policy is not boolean");
      quit();
    }
  },
  cloak: cloak
...
function cloak(label, highValue, lowValue) {
  ...
  let pc = getPC();
  let lab = new Label(label);
  if (pc.contains(lab))
    return highValue;
  else if (pc.contains(lab.reverse()))
    return lowValue;
  else
    return new FacetedValue(new Label(label), highValue, lowValue);
}

// Functions that can be used by programs to incorporate faceted Behaviour.
facetedGlobalBase.__proto__.PolicyLibrary = PolicyLibrary;
